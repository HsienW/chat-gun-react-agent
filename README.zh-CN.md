...
Chat Gun 是一套以 React、TypeScript 与 LangGraph JS 构建的全栈 Agent Chat 应用。它支持多 Agent 对话、串流回复、Tool Calling、图片输入与 Human-in-the-Loop，并通过 BFF 统一处理浏览器与 LangGraph Runtime 之间的 API 流量。
