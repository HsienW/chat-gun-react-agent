# agent-interaction-runtime Specification

## Purpose
Business Interaction Runtime：定義 active run 期間收到新輸入時的業務策略、單一權威 run 的 ownership 與 generation 隔離、輸入分類、side-effect-aware cancellation 與互動生命週期事件。TBD - created by archiving change add-agent-interaction-runtime.
