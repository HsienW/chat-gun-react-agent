...
Defines the backend Opik agent-tracing capability: environment-gated client initialization with graceful degradation, the agent → LangGraph node → LLM/tool trace hierarchy, correlation ID mapping, redaction before export, coexistence with X8 OTel, parallel trace isolation, and non-interfering instrumentation points.
