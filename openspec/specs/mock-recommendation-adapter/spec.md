...
本規格定義以最小 Mock Domain 驗證 Recommendation Framework 業務解耦邊界的正式需求，涵蓋結構化 intent、候選檢索、hard／soft constraint、scope 投影、全鏈整合與 Core 零業務依賴。
...
THEN MUST fail-closed 拒絕該候選
AND MUST NOT 產出跨 tenant 的 recommendation card

---

### Requirement: 框架 Core MUST 保持零業務 import 與零變更

X9 Recommendation Framework Core（`backend/src/recommendation/`）MUST 保持零 Mock 業務 import、零 Mock 常數與零修改；Mock Adapter MUST 完全位於獨立注入模組。

#### Scenario: Core 零 Mock import

GIVEN Mock Recommendation Adapter 已完成
WHEN 檢視 `backend/src/recommendation/` 的 imports、types 與常數
THEN MUST NOT import `mock-recommendation`
AND MUST NOT 包含 `mock` domain 或 `category`／`color`／`price` 業務常數
