...
本規格定義 X10.1 長期記憶治理（Long-Term Memory Governance）的正式需求。production memory persistence 採 LangGraph Store 邊界並以 project-owned `MemoryStorePort` 封裝（production adapter 為 `PostgresStore`、`InMemoryStore` 僅供 deterministic tests）；記憶讀取／寫入／刪除皆以 X8.7 `authorize()` 為單一授權來源，namespace 僅為路由鍵、非安全邊界。讀取採自動召回（`MemoryContextProvider` 呼叫 `recall()`，結果以 priority P3 注入 context），寫入採政策式（runtime-validated `MemoryCandidate` 經 `MemoryWritePolicy` 與 write authorization 後 commit），涵蓋 optimistic concurrency、bi-temporal 有效性、relation 分類、retention／deletion／tenant isolation 與降級失敗語意。X10.1 不提供 planner-controlled memory Tool。
