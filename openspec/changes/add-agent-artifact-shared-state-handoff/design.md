...
- 未知 `kind` MUST 視為 `unknown`，保留原始 payload，不可假設 shape。
...
| `terminalStatus` | `NON_TERMINAL` 或 `TERMINAL`（進入 COMPLETED/FAILED/INCOMPLETE） |
...
- 以 `openspec/`、`.agent-runtime/` 或專案根目錄下的已知安全前綴開頭。
- 不包含 `..`。
