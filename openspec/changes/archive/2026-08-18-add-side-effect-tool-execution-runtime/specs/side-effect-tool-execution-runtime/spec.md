# Specs：add-side-effect-tool-execution-runtime
...
`tool-governance` 的 `governedInvoke` 目前把所有例外轉成 `Error: <tool> failed by tool governance - ...` 字串。本次 MUST 新增 `executeTyped()` 回傳 `GovernedToolOutcome`，並保留 `governedInvoke` 作為最外層 legacy LangChain 相容 adapter；Runner 一律使用 `executeTyped()`，不解析錯誤字串。

#### Scenario: executeTyped 與 governedInvoke 並存

GIVEN `tool-governance` 既有 `governedInvoke` 回傳字串錯誤
WHEN 本次修改
THEN MUST 新增 `executeTyped()` 回傳 typed outcome
...
`bff-stream-proxy` 已透傳 `x-idempotency-key`，但 CORS `access-control-allow-headers` 未包含它，且無長度／格式／重複 header 驗證。本次 MUST 加入 CORS header、驗證與 trusted namespace，並定義 canonical 名稱與 TTL 設定。
...
既有 `saga-orchestrator` 的 `resolveOverallStatus` 在 action 失敗後仍回傳 `partial_failure` 且把 step 標成 `compensated`。本次 MUST 加入 `manual_intervention_required` 語意，且 action 失敗時不得再標 step 為 `compensated`。
