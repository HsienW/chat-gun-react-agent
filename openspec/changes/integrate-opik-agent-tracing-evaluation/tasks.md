...
- [ ] **前置步驟**：確認 `opik` npm package 存在、版本號、TypeScript type export，並記錄實際 API 表面與 design interface（OpikClient、OpikTrace、OpikSpan）的 mapping。若 package 不存在或不支援 TypeScript，觸發 OTel export fallback 路徑。
- [ ] 在 `backend/package.json` 新增 `opik` dependency（`^1.x`）
- [ ] 在 `backend/src/platform/runtime-config.ts` 新增 Opik 相關 env key 與 type
  - `OPIK_ENABLED`、`OPIK_API_KEY`、`OPIK_WORKSPACE`、`OPIK_HOST`、`OPIK_PROJECT_NAME`、`OPIK_REDACT_ENABLED`
- [ ] 所有 Opik env key 有預設值、型別與註解
...
- [ ] 建立 `backend/src/platform/tracing/opik/opik-setup.ts`
...
- [ ] Opik client 使用 dynamic import（`import("opik")`），避免 import-time side effect
- [ ] 建立 `backend/src/platform/tracing/opik/opik-setup.test.ts`
...
- [ ] 建立 `backend/src/platform/tracing/opik/opik-redaction.ts`
...
- [ ] 複用 `span-manager.ts` 的 `sanitizeErrorMessage` 邏輯
- [ ] 建立 `backend/src/platform/tracing/opik/opik-redaction.test.ts`
  - API key 被 redact
  - Prompt 被替換為 hash
  - PII 被遮蔽
  - Correlation ID 保留
...
- [ ] 建立 `backend/src/platform/tracing/opik/opik-tracer.ts`
...
- [ ] Span metadata 包含 `threadId`、`runId`、`taskId`、`stepId`、`toolCallId`
- [ ] 透過 Feature Flag（`OPIK_ENABLED`）切換 NoopOpikTracer 與 RealOpikTracer
- [ ] 建立 `backend/src/platform/tracing/opik/opik-tracer.test.ts`
...
- [ ] 在 LangGraph agent graph entry 呼叫 `OpikTracer.traceAgentRun()`
- [ ] 在關鍵 node 中接入 `withNodeSpan()`
- [ ] 在 `TracedChatModelInvoker`（`llm-gateway.ts`）加入 Opik span（與既有 OTel span 並存）
- [ ] 在 Tool execution wrapper 加入 Opik span
- [ ] 確保 Opik span 與 OTel span 互不干擾（各自獨立建立 context）
...
- [ ] 建立 `backend/src/evaluation/opik/dataset.ts`
...
- [ ] 建立 `backend/src/evaluation/opik/dataset.test.ts`
...
- [ ] 建立 `backend/src/evaluation/opik/metrics/tool-call-correctness.ts`
  ...
  - Score 0.0–1.0
- [ ] 建立 `backend/src/evaluation/opik/metrics/tool-call-correctness.test.ts`
  - 完全符合 → 1.0
  - 城市不符合 → < 1.0
  - 無 tool call → 0.0
...
- [ ] 建立 `backend/src/evaluation/opik/metrics/response-quality.ts`
...
- [ ] 建立 `backend/src/evaluation/opik/metrics/response-quality.test.ts`
  - 高品質回應 → >= 0.8
  - 不相關回應 → < 0.5
...
- [ ] 建立 `backend/src/evaluation/opik/experiment.ts`
...
- [ ] 支援相同 dataset version 但不同 agent config 的兩次 experiment 比較
- [ ] 建立 `backend/src/evaluation/opik/experiment.test.ts`
  - 相同 config → 結果可比較
  - 不同 model → 可並排比較
...
- [ ] 建立 `docs/decisions/opik-integration-assessment.md`
- [ ] 記錄以下維度：
...
- [ ] 明確建議：採用（production + dev）、僅保留為 dev tool、或拒絕
- [ ] 若建議採用，記錄 migration path（從 trial 到 production 的條件）
