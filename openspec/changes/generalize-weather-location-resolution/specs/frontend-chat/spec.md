...
Frontend MUST 依 storage mode 顯示 clarification：Permanent 可顯示有限候選；Temporary 只能顯示不含 Provider 候選的通用補充提示。

#### Scenario: 顯示候選地點

- GIVEN Permanent Weather Tool Result 包含多個 Candidates
...
#### Scenario: Temporary durable result 不含候選

- GIVEN Frontend 收到 `schemaVersion: "2.0"`、`geocodingStorageMode: "temporary"` 與 `status: "needs_clarification"`
- AND durable result 不含 Candidates
- WHEN Frontend Render Tool Panel
- THEN Frontend MUST 顯示一般補充地點提示
- AND MUST NOT 將缺少 Candidates 視為 malformed result
...
- GIVEN Weather Tool Result 的 `schemaVersion` 為 `2.0`
...
#### Scenario: Temporary projection violation

- GIVEN Frontend 收到 `status: "error"` 與 `code: "weather_temporary_projection_violation"`
- WHEN Frontend Render Tool Panel
- THEN Tool Call MUST 進入 Terminal State
- AND MUST 顯示安全服務錯誤文案
- AND MUST NOT 永久顯示「執行中」
- AND MUST NOT 顯示或記錄原始違規 payload
