...
本 capability 定義專案「對外身分 token」的統一命名契約，作為縮名 `chat-gun-react-agent` → `chat-gun` 的驗收基準。本 change 為靜態身分改名，無執行期逾時／取消／重試語意；Scenario 以「一致性」與「不殘留舊名」作為可驗證的成功／失敗邊界。

## ADDED Requirements

### Requirement：專案 slug 與顯示名稱

專案對外識別名稱統一為：slug `chat-gun`、顯示名稱 `Chat Gun`。
...
backend、bff、frontend 三個 workspace 的 package 名稱與其 lockfile 根名稱必須一致，且使用 `chat-gun-*` 前綴。
...
backend 對外部 HTTP API 發起工具呼叫時，User-Agent 標頭必須使用 `chat-gun/<version>`，不得使用舊名稱。
...
backend 連接 MCP Server 時，client name 必須為 `chat-gun-<serverName>`。
...
OpenTelemetry service name 與 Opik project name 的預設值必須為 `chat-gun`。
...
backend 與 bff 的 Docker image 名稱分別為 `chat-gun` 與 `chat-gun-bff`。
