...
async function retryWeatherPlannerExtraction(
  ...
  const prompt = [
    ...
    "Use the complete user-provided place text, including multi-level administrative qualifiers (e.g. '台灣高雄大寮' stays as one location string, not broken into pieces).",
    "Keep weather.location as the full original user-provided place text. Do not decompose a composite place name into separate fields or omit administrative levels.",
...
async function planResearch(
  ...
  const prompt = [
    ...
    "For weather, extract the complete place text the user provided, including any country, province, city, district, or neighborhood qualifiers (e.g. '台灣高雄大寮', '北京市朝陽區'). Keep weather.location as the full original place text the user used. Do not decompose a multi-level administrative place name into separate fields or omit levels. Include country or region only when the user explicitly mentioned them.",
